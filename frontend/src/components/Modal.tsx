...
export const Modal: FC<Props> = ({
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      const updatedMeds = await getMedications(userId);
      setMeds(updatedMeds);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-20 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="relative bg-white w-full max-w-xl mx-4 p-8 rounded-xl shadow-xl">
        ...
        <div className="space-y-2 mb-4 text-sm text-gray-700">
          ...
          <p>
            <strong>Schedule:</strong> {scheduleType} – {times}x
          </p>
          ...
          {scheduleType === "monthly" && dayOfMonth && (
            <p>
              <strong>Day of Month:</strong> {dayOfMonth}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`px-5 py-2 rounded-md transition text-white hover:cursor-pointer ${
              isFutureDate
